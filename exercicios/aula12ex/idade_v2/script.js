function checar() {
    ...
    var dataNascimento = new Date(Number(ano.value), Number(mes.value), Number(dia.value))
    ...
    if (ano.value === '' || mes.value === '' || dia.value === '' || Number(ano.value) > ano_atual || Number(mes.value) < 1 || Number(mes.value) > 12 || Number(dia.value) < 1 || Number(dia.value > 31)) {
     ...
     if (sexo[0].checked) {
        ...
        foto.innerHTML = `Detectamos um(a) ${genero} com <strong>${anos} anos</strong> de idade.<br>`
        foto.innerHTML += `Essa já viveu:<br>`
        foto.innerHTML += `${mesesVividos} meses<br>${semanasVividas} semanas<br>${diasVividos} dias`
